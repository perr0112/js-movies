...
describe('updateMoviesElt', () => {
  beforeEach(() => {
    document.body.innerHTML = "<article class='movies-list'></article><nav class='pagination'></nav>";
  ...
  test('insert 2 .movie-item in the container', () => {
    updateMoviesElt();
    getAllMovies.mock.results[0].value.then(() => expect(document.body.querySelectorAll('.movie-item').length).toBe(2));
  });
